...
runTest('.sort(compareFunction)', () => getRandomArray().sort((a, b) => a - b)) //   137 ms, 这个结果有点意外，提供自定义比较函数，没慢反而快了
...
function binarySearch (arr, item) {
  ...
  while (low <= heigh) {
    ...
    element = arr[mid]
    if (element < item) { low = mid + 1 } else if (element > item) { heigh = mid - 1 } else { return mid }
  }
  return -1
}
