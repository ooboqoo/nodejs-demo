...
const LoggerModule = rewire('./logger')

const stringify = LoggerModule.__get__('stringify')
...
describe('log._write()', () => {
  ...
  test('only write error to errOutput', () => {
    ...
    expect(errOutput.length).toBe(2)
  })
...
describe('log._normalize()', () => {
  test('can normalize params with err', () => {
    ...
    expect(log._normalize(err))
      .toEqual({title: '', content: '', err})
    expect(log._normalize('my content', err))
  ...
  test('can normalize params without err', () => {
    expect(log._normalize('my content'))
      .toEqual({title: '', content: 'my content'})
    expect(log._normalize({message: 'my content'}))
      .toEqual({title: '', content: {message: 'my content'}})
  })
})
...
describe('log.useTempLevel()', () => {
  ...
  test('can temporarily change the log level', () => {
    ...
    childLog.info('my message')
    expect(logOutput.length).toBe(0)
    childLog.warn('my message')
    expect(logOutput[0].substring(24)).toBe('WARN [] : "my message"')
  })
...
describe('log.error()', () => {
  ...
  test('can log Error object as expected', () => {
    ...
    expect(errOutput[0].substring(24, 114))
      .toBe('ERROR [] : {"message":"x is not defined","stack":"ReferenceError: x is not defined\\n    at')
    log.error('my title', 'my content', err)
    expect(errOutput[1].substring(24, 109))
      .toBe('ERROR [] my title: {"message":"my content, x is not defined","stack":"ReferenceError:')
  })
  ...
  test('can log error string as expected', () => {
    ...
    expect(errOutput[2].substring(24))
      .toBe('ERROR [] my title: {"message":"my content","stack":"Error: my error message"}')
  })
})
